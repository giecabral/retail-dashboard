...
export default function handler(req: NextApiRequest, res: NextApiResponse<AgeGroupSale[] | { error: string }>) {
  ...
  try {
    ...
    console.error('[sales-by-age-group]', err)
    res.status(500).json({ error: 'Failed to load sales by age group' })
  }
}
